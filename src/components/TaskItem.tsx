...
export function TaskItem({ task, onToggle, onDelete, onEdit }: TaskItemProps) {
  ...
  const handleSave = () => {
    if (editTitle.trim()) {
      onEdit(task.id, editTitle, editDate || null);
    }
    setEditing(false);
  };
    ...
    <div className={`group bg-card rounded-xl border border-border p-3 flex items-start gap-3 transition-all duration-200 hover:shadow-sm ${exiting ? "task-exit" : "task-enter"}`}>
      <button
        ...
        className={`mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
          task.completed
            ? "bg-primary border-primary check-pop"
      ...
      <div className="flex-1 min-w-0">
        {editing ? (
          ...
          <>
            ...
            {dueDateLabel && (
              <span className={`inline-flex items-center gap-1 mt-1 text-xs font-medium ${isOverdue ? "text-destructive" : "text-muted-foreground"}`}>
      ...
      {!editing && (
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={() => { setEditTitle(task.title); setEditDate(task.dueDate || ""); setEditing(true); }}
